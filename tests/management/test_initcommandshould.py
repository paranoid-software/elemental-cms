...
class TestInitCommandShould:
    ...
    def test_exit_early_when_project_is_already_initialized(self, settings_fixture):
        runner = CliRunner()
    ...
    def test_create_default_folder_structure(self, settings_fixture):
        runner = CliRunner()
        with runner.isolated_filesystem():
            os.makedirs('settings')
            with open('settings/debug.json', 'w') as f:
            ...
            assert os.path.exists(f'static/{settings_fixture["cmsCoreContext"]["APP_NAME"]}')
    ...
    def test_show_success_feedback(self, settings_fixture):
        runner = CliRunner()
        with runner.isolated_filesystem():
            os.makedirs('settings')
            with open('settings/debug.json', 'w') as f:
