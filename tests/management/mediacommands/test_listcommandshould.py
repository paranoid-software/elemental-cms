...
class TestListCommandShould:
    ...
    def test_fail_when_folder_argument_is_missing(self, default_elemental_fixture, default_settings_fixture):
        runner = CliRunner()
        with runner.isolated_filesystem():
    ...
    def test_display_zero_files_feedback_message(self, default_elemental_fixture, default_settings_fixture):
    ...
    def test_display_complete_files_list(self, default_elemental_fixture, default_settings_fixture):
        file_names_list = [
        ...
        with EphemeralGcsContext(initial_state=[
            ...
            with runner.isolated_filesystem():
                with EphemeralElementalFileSystem(default_elemental_fixture, default_settings_fixture, [
                    (os.path.join('media', 'readme.md'), '# README')
                ]):
                    # noinspection PyTypeChecker
                    result = runner.invoke(cli, ['media', 'list', '--all'])
                    [assert_that(result.output).contains(file_name) for file_name in file_names_list]

    def test_display_folder_files_list(self, default_elemental_fixture, default_settings_fixture):
