...
class DraftsRepository(MongoDbConnectionManager):

    def __init__(self, db_context: MongoDbContext):
        self.__coll = self._get_db(db_context).get_collection('drafts')
    ...
    def update(self, _id, spec, upsert=True) -> bool:
        if not ObjectId.is_valid(_id):
            return False
        update_result = self.__coll.replace_one({'_id': ObjectId(_id)}, spec, upsert=upsert)
